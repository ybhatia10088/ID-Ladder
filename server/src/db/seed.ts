/**
 * Seeds the ID-Ladder demo database from scratch.
 *
 * Idempotent: the database file is deleted and rebuilt on every run, so
...
const SEEDED_AT = "2026-07-30T00:00:00.000Z";
...
const caseHoldings: { case_id: string; document_id: string }[] = [];

function seed(): void {
  // Drop and rebuild so repeated runs converge on identical state.
  ...
  try {
    ...
    const insertHolding = db.prepare(
      `INSERT INTO case_holdings (case_id, document_id) VALUES (@case_id, @document_id)`,
    );

    db.transaction(() => {
      for (const row of documents) insertDocument.run(row);
      for (const row of prerequisites) insertPrerequisite.run(row);
      for (const row of organizations) insertOrganization.run(row);
      for (const row of cases) insertCase.run(row);
      for (const row of caseHoldings) insertHolding.run(row);
    })();
    ...
    const counts = {
      ...
      cases: cases.length,
      case_holdings: caseHoldings.length,
    };
